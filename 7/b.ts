...
let folderToDelete: Folder = root;

const totalSize = getFolderSize(root);

const freeSpace = 70000000 - totalSize;

const neededSpace = 30000000 - freeSpace;

const getSmallestFolderToDelete = (folder: Folder) => {
  const folderSize = getFolderSize(folder);
  const currentFolderSize = getFolderSize(folderToDelete);
  if (folderSize >= neededSpace && folderSize < currentFolderSize) {
    folderToDelete = folder;
  }
  for (let i = 0; i < folder.children.length; i++) {
    const element = folder.children[i];
    if (element.type === "folder") {
      getSmallestFolderToDelete(element);
    }
  }
};

getSmallestFolderToDelete(root);

console.log("result: ", getFolderSize(folderToDelete));
